...
Reproduction.prob<-function(x,params) {
  u = exp(params$prob.repro.int + 
        params$prob.repro.slope * x)
 
  return(u / (1 + u))
}
# Fecundity
SB.Go<-function(x, params, fec.model) {
...
grow.prob.trunc<-function(size2,size1,model,lower.size.bound,upper.size.bound){
  ...
  return(dtrunc(size2, spec = 'norm',
                a = lower.size.bound, b = upper.size.bound,
                mean = size2.pred, sd = sd.mod))
}
