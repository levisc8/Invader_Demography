...
PopData1 <- filter(Ailanthus4R, Trt == 'Control' | Trt == 'All')
PopData <- filter(PopData1, Burn != "Y")

# PopDataSDL = Ailanthus4R[Ailanthus4R$Trt == 'Control' &
#                            Ailanthus4R$Burn != 'Y' & 
#                            Ailanthus4R$Stage2013 == 'SDL', ]

PopDataSDL <- filter(Ailanthus4R, Trt == 'Control' &
                     Burn != 'Y' &
                     Stage2013 == 'SDL')
...
# Nonclones = Good_PopData[Good_PopData$Stage2012 !='gone' & 
#                            Good_PopData$Stage2013 != 'SDL' &
#                            Good_PopData$Stage2012 != 'SDL', ]

Good_PopData <- filter(PopData, Quality2013 == 'good')
Nonclones <- filter(Good_PopData, Stage2012 != 'gone' &
                      Stage2013 != 'SDL' &
                      Stage2012 != 'SDL')
...
Clones <- filter(PopData, Stage2013 != 'gone') %>% 
  filter(Stage2013 != 'SDL' & Stage2012 != 'SDL') %>%
  filter(Stage2012 == 'gone' & Quality2013 == 'good')
        
# PopDataRA1 = PopData[PopData$Stage2013 != 'RA',] 
# PopDataRA = PopDataRA1[PopDataRA1$Stage2013 != 'Boy',]
# PopDataSeeds = Ailanthus4R[Ailanthus4R$Stage2013 == 'RA' | 
#                              Ailanthus4R$Stage2013 == 'girl',]

PopDataRA <- filter(PopData, Stage2013 != 'RA') %>% filter(Stage2013 != 'Boy')
PopDataSeeds <- filter(Ailanthus4R, Stage2013 == 'RA' | Stage2013 == 'girl')
...
Germ <- filter(Ailanthus4R_Germ, Study == 'Germ')
Viable <- filter(Ailanthus4R_Germ, Study == 'Viable')
FireY <- filter(Ailanthus4R_Germ, Study == 'Fire' &
                  Location == '0' & Burn == 'Yes')
FireN <- filter(Ailanthus4R_Germ, Study == 'Fire' &
                  Location == '0' & Burn == 'No')
...
SDL_NoBurn <- filter(AASeedlings, Burn == 'N')
...
# and fecundity, respectively. For the numerical integration, we are using the midpoint 
...
ContN_lambda
# Right eigenvector
w.eigen = Re(eigen(K_ContN)$vectors[,1])  # Stable stage distribution
stable.dist = w.eigen/sum(w.eigen)  # Standardize to total density
# Left eigenvector
v.eigen = Re(eigen(t(K_ContN))$vectors[,1])
repro.val = v.eigen/v.eigen[1]      # Reproductive value

### Sensitivity and elasticity matrices ###
v.dot.w=sum(stable.dist*repro.val)*h
sens=outer(repro.val,stable.dist)/v.dot.w
elas_ContN=matrix(as.vector(sens)*as.vector(K_ContN)/ContN_lambda,nrow=n.size+1)


# Delete discrete stages from K_ContN (i.e., subset) so transitions can be graphed by size.
K_ContN1=K_ContN[1:(n.size),1:(n.size)]

lambda1 = Re(eigen(K_ContN1)$values[1])
lambda1

# Right eigenvector
w.eigen1 = Re(eigen(K_ContN1)$vectors[,1])  # Stable stage distribution
ContN_stable.dist1 = w.eigen1/sum(w.eigen1)  # Standardize to total density
# Left eigenvector
v.eigen1 = Re(eigen(t(K_ContN1))$vectors[,1])
ContN_repro.val1 = v.eigen1/v.eigen1[1]      # Reproductive value

### Sensitivity and elasticity matrices ###
v.dot.w1=sum(ContN_stable.dist1*ContN_repro.val1)*h
v.eigen1 = Re(eigen(t(K_ContN1))$vectors[,1])
ContN_sens1=outer(ContN_repro.val1,ContN_stable.dist1)/v.dot.w1
v.eigen1 = Re(eigen(t(K_ContN1))$vectors[,1])
ContN_elas1=matrix(as.vector(ContN_sens1)*as.vector(K_ContN1)/lambda1,nrow=n.size)