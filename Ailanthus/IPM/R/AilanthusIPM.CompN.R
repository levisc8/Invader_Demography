...
CompN_lambda
# Right eigenvector
w.eigen = Re(eigen(K_CompN)$vectors[,1])  # Stable stage distribution
stable.dist = w.eigen/sum(w.eigen)  # Standardize to total density
# Left eigenvector
v.eigen = Re(eigen(t(K_CompN))$vectors[,1])
repro.val = v.eigen/v.eigen[1]      # Reproductive value

### Sensitivity and elasticity matrices ###
v.dot.w=sum(stable.dist*repro.val)*h
sens=outer(repro.val,stable.dist)/v.dot.w
elas_CompN=matrix(as.vector(sens)*as.vector(K_CompN)/CompN_lambda,nrow=n.size+1)

# Delete discrete stages from K_CompN (i.e., subset) so transitions can be graphed by size.
K_CompN1=K_CompN[1:(n.size),1:(n.size)]

lambda1 = Re(eigen(K_CompN1)$values[1])
lambda1

# Right eigenvector
w.eigen1 = Re(eigen(K_CompN1)$vectors[,1])  # Stable stage distribution
CompN_stable.dist1 = w.eigen1/sum(w.eigen1)  # Standardize to total density
# Left eigenvector
v.eigen1 = Re(eigen(t(K_CompN1))$vectors[,1])
CompN_repro.val1 = v.eigen1/v.eigen1[1]      # Reproductive value

### Sensitivity and elasticity matrices ###
v.dot.w1=sum(CompN_stable.dist1*CompN_repro.val1)*h
v.eigen1 = Re(eigen(t(K_CompN1))$vectors[,1])
CompN_sens1=outer(CompN_repro.val1,CompN_stable.dist1)/v.dot.w1
v.eigen1 = Re(eigen(t(K_CompN1))$vectors[,1])
CompN_elas1=matrix(as.vector(CompN_sens1)*as.vector(K_CompN1)/lambda1,nrow=n.size)