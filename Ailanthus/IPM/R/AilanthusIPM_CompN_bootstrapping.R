...
### Start bootstrapping
CompN_bootstrap_clones = rep(NA,1000)
CompN_bootlambda = rep(NA, 1000) # This will save all the lambdas from each bootstrap. For now I fill in a column with 1000 rows with the value 'NA'.  Later they are replacee with data.
for(j in 1:1000) 
{
  ...
  PopDataSDL = subset(Ailanthus4R, (Trt != 'Herb' & Trt != 'Control' & Burn != 'Y' & Stage2013 == 'SDL'))
  ...
  CompN_bootstrap_clones[j] = p.vec$clonal.prob
  CompN_bootlambda[j] = CompN_lambda_boot
}

CompN_bootstrap_clones = sort(CompN_bootstrap_clones)
CompN_clones_CI = c(CompN_bootstrap_clones[25], CompN_bootstrap_clones[975])

CompN_bootlambda = sort(CompN_bootlambda)

CompN_lambda_CI = c(CompN_bootlambda[25], CompN_bootlambda[975])

### End bootstrapping, clean up for next model
files.rm <- ls()
keep.files <- c('CompN_lambda', 'ContN_lambda',
                'CompN_lambda_CI', 'ContN_lambda_CI')
files.rm <- files.rm[!files.rm %in% keep.files]
rm(list = files.rm)
