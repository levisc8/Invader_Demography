...
# This looks hideous, but it just makes sure that each plant gets assigned the 
# correct stage class

for(i in 1:dim(PopData)[1]) {
  if(PopData$Stage[i] == 'SDL') {
    PopData$StageMatMod[i] <- 'SDL'
  } else if(PopData$Stage[i] == 'SDL2') {
    PopData$StageMatMod[i] <- 'SDL2'
  } else if(PopData$Stage[i] == 'Adult') {
    if(PopData$Size[i] > 8 &&
       PopData$Size[i] <= 250) {
      PopData$StageMatMod[i] <- 'Small'
    } else if(PopData$Size[i] > 250 &&
              PopData$Size[i] < 1000) {
...
for(i in 1:dim(PopData)[1]) { 
  if(PopData$Survival[i] == 0 |
    ...
    PopData$StageMatModNext[i] <- NA
            ...
            is.na(PopData$StageNext[i]) |
            is.na(PopData$SizeNext[i])) { 
    if(is.na(PopData$SizeNext[i])){
      
    # if it didn't die, but we don't have any other info, stageNext is same Stage
      PopData$StageMatModNext[i] <- PopData$StageMatMod[i]
      
    } else {
      PopData$StageMatModNext[i] <- PopData$StageNext[i]
    }
    # And the rest implements the size based classification
  } else if(PopData$StageNext[i] == 'SDL2') {
    PopData$StageMatModNext[i] <- 'SDL2'
  } else if(PopData$StageNext[i] == 'Adult') {
    if(PopData$SizeNext[i] <= 250) {
      PopData$StageMatModNext[i] <- 'Small'
    } else if(PopData$SizeNext[i] > 250 &&
              PopData$SizeNext[i] < 1000) {
...
A_CR <- matrix(
  c(
    0, 0, 0, CR_F_Small * (1 - meanGerm) * meanSV, CR_F_Med * (1 - meanGerm) * meanSV, CR_F_Large * (1 - meanGerm) * meanSV,
...
A_Cont <- matrix(
  c(
    0, 0, 0, Cont_F_Small * (1 - meanGerm) * meanSV, Cont_F_Med * (1 - meanGerm) * meanSV, Cont_F_Large * (1 - meanGerm) * meanSV,
...
# Split into different classes and treatments
...
for(i in 1:nResamp) {
  ...
  # Initialize resamplers for each stage class x trt combo. Additionally,
  # if we don't end up with an adequate number of large adults for the fecundity
  # calculation, the if(all(...)) adds one of  them back in
...
# This is a programmatic approach so that we don't have to write the name of every
# vector
...
results$yints[results$VitalRate == 'lambda'] <- 1
