...
PopData1 <- Ailanthus4R[Ailanthus4R$Treatment == 'Control' | Ailanthus4R$Treatment == 'All', ]
PopData <- PopData1[PopData1$Burn != 'Y', ]

PopDataSDL <- Ailanthus4R[Ailanthus4R$Treatment == 'Control' & 
                            Ailanthus4R$StageNext == 'SDL' & 
                            Ailanthus4R$Burn != 'Y', ]
...
FireY <- Ailanthus4R_Germ[Ailanthus4R_Germ$Study == 'Fire' & Ailanthus4R_Germ$Burn == 'Yes',]
...
for(j in 1:1000) 
{
  ...
  PopDataSDL = subset(Ailanthus4R, (Treatment != 'Herb' & Treatment != 'Comp' & Burn != 'Y' & StageNext == 'SDL'))
  ...
  # Survival vector. Occasionally, the computation of the linear term yields Inf.
  # This results in NaNs in S_ContN. Replace those w/ 1s
  S_ContN <- s.x.CONTN(y, params = p.vec) 
  S_ContN[is.na(S_ContN)] <- 1 
  SMatrix <- array(0, dim = c(100,99))
  S_ContN1 <- cbind(S_ContN, SMatrix)          
  ...
  # Calculate lambda
  ContN_bootlambda[j] <- Re(eigen(K_ContN)$values[1])
  p.vec.all <- lapply(seq_along(p.vec.all), function(x) p.vec.all[[x]] <- c(p.vec.all[[x]], p.vec[1, x]))
  
}

names(p.vec.all) <- names(p.vec)
p.vec.all$lambda <- c(ContN_lambda, ContN_bootlambda)

saveRDS(p.vec.all, file = 'Ailanthus_IPM/Cont_Bootstrap_Output.rds')
