...
sdlsurv <- read.csv("Potentilla_MPM/Potentilla_SDL_Clean.csv")
...
params <- ks %>%
  ...
  summarise(N = length(Treatment),
            ...
            NRA.15 = length(Treatment[StageNext == "NRA"]),
            RA.15 = length(Treatment[StageNext == "RA"]),
            N15 = NRA.15 + RA.15,
            pf = RA.15 / N15) %>%
  as.data.frame()
...
# A quick reference guide for vital rates. More detailed descriptions are
# in the manuscript.
...
s1b_c <- params[params$Stage == "SDL" &
                  params$Treatment == "Control", "Survival"]
s1_c <- s1a_c * s1b_c
s2_c <- params[params$Stage == "NRA" &
                 params$Treatment == "Control", "Survival"]
s3_c <- params[params$Stage == "RA" & 
                 params$Treatment == "Control", "Survival"]
pfsdl_c <- params[params$Stage == "SDL" &
                    params$Treatment == "Control", "pf"]
pfnra_c <- params[params$Stage == "NRA" &
                    params$Treatment == "Control", "pf"]
pfra_c <- params[params$Stage == "RA" &
                   params$Treatment == "Control", "pf"]
f_c <- mean(ks[ks$Treatment == "Control", "Seeds"], na.rm = TRUE)

# Comp Rem
s1a_cr <- mean(crsdl$Survival,na.rm=T)
s1b_cr <- params[params$Stage == "SDL" &
                   params$Treatment == "Comp", "Survival"]
s1_cr <- s1b_cr * s1a_cr
s2_cr <- params[params$Stage == "NRA" &
                  params$Treatment == "Comp", "Survival"]
s3_cr <- params[params$Stage == "RA" &
                  params$Treatment == "Comp", "Survival"]
pfsdl_cr <- params[params$Stage == "SDL" &
                     params$Treatment == "Comp", "pf"]
pfnra_cr <- params[params$Stage == "NRA" &
                     params$Treatment == "Comp","pf"]
pfra_cr <- params[params$Stage == "RA" &
                    params$Treatment == "Comp","pf"]
...
####################################################
# Bootstrapping code
# We bootstrap the data set 1000 times, creating a new matrix for each one, then store
...
for(j in 1:nreps) {
  ...
  s1b_c <- params1[params1$Stage == "SDL" &
                     params1$Treatment == "Control", "Survival"]
  s1_c <- s1a_c * s1b_c
  s2_c <- params1[params1$Stage == "NRA" & 
                    params1$Treatment == "Control", "Survival"]
  s3_c <- params1[params1$Stage == "RA" & 
                    params1$Treatment == "Control", "Survival"]
  pfsdl_c <- params1[params1$Stage == "SDL" & 
                       params1$Treatment == "Control", "pf"]
  pfnra_c <- params1[params1$Stage == "NRA" &
                       params1$Treatment == "Control", "pf"]
  pfra_c <- params1[params1$Stage == "RA" &
                      params1$Treatment == "Control", "pf"]
  ...
  s1b_cr <- params1[params1$Stage == "SDL" &
                      params1$Treatment == "Comp", "Survival"]
  s1_cr <- s1a_cr * s1b_cr
  s2_cr <- params1[params1$Stage == "NRA" &
                     params1$Treatment == "Comp", "Survival"]
  s3_cr <- params1[params1$Stage == "RA" &
                     params1$Treatment == "Comp", "Survival"]
  pfsdl_cr <- params1[params1$Stage == "SDL" &
                        params1$Treatment == "Comp", "pf"]
  pfnra_cr <- params1[params1$Stage == "NRA" &
                        params1$Treatment == "Comp", "pf"]
  pfra_cr <- params1[params1$Stage == "RA" &
                       params1$Treatment == "Comp", "pf"]
...
# Reshape the data so that it's ggplot2 friendly
PlotData <- results %>%
